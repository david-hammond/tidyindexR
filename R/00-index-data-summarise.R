...
index_data_summarise = function(df){
  df = df %>% dplyr::group_by(.data$variablename) %>%
    dplyr::summarise(num_geos = length(unique(.data$geocode)),
                     ...
                     lowest_score = paste(paste(unique(.data$country[.data$value == min(.data$value)])[1], collapse = ","), round(min(.data$value),2)),
                     highest_score = paste(paste(unique(.data$country[.data$value == max(.data$value)][1]), collapse = ","), round(max(.data$value), 2)),
